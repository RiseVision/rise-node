...
export default {
  ...
  aggregateBlocksReward(params: { start?: number, end?: number }) {
    return [
      'WITH',
      'delegate AS (SELECT',
  ...
  list(params: { where: string[], sortField?: string, sortMethod?: 'ASC' | 'DESC' }) {
    return [
  ...
  getCommonBlock(params: { previousBlock?: string }) {
    return [
