...
describe('helpers/bignum', () => {
  describe('fromBuffer', () => {
    it('should return a BigNumber', () => {
      const bn = MyBigNumb.fromBuffer(
        Buffer.from('11223344556677889900ff', 'hex')
      );
    ...
    it('should throw error when buffer length and size are not multiples', () => {
      expect(() => {
        MyBigNumb.fromBuffer(Buffer.from('11223344556677889900ff', 'hex'), {
          size: 3,
        });
    ...
    it('should return the expected value', () => {
      ...
      const hash = crypto
        .createHash('sha256')
        .update('Testing helpers/bignum.ts')
        .digest();
      const buf = MyBigNumb.fromBuffer(hash);
      expect(buf.toString(16)).to.be.deep.equal(
        '298060b98876a71d422b03ea601aee4625eb883bdd317e7556dbe4cbb0ab57d3'
      );
    });
  });
