...
export function configCreator(
  ...
  const schemas = [{ module: 'main', schema: configSchema.config }].concat(
    modules
      .filter((m) => typeof m.configSchema !== 'undefined')
  ...
  for (const { module, schema } of schemas) {
    ...
    if (!valid) {
      console.log(
        `Failed to validate config data for module ${module}`,
