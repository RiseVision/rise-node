...
export function loggerCreator(config: any = {}): ILogger {
  ...
  Object.keys(config.levels).forEach((name: string) => {
    function log(message, data) {
      if (
        config.levels[config.errorLevel] > config.levels[name] &&
      ...
      logData.symbol = config.level_abbr[logData.level]
        ? config.level_abbr[logData.level]
        : '???';
      ...
      if (
        config.echo &&
        config.levels[config.echo] <= config.levels[logData.level]
      ) {
        if (logData.data) {
          console.log(
            '[' + logData.symbol.bgYellow.black + ']',
            logData.timestamp.grey,
            '|',
            elapsed.toString().cyan,
            '|',
            logData.message,
            '-',
            logData.data
