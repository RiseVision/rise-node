...
describe('helpers/sequence', () => {
  ...
  describe('tick', () => {
    ...
    it('promise rejection should make the sequence advance to next task', async () => {
      ...
      sequence
        .addAndPromise(getPromiseWorker(new Error('rejecting'), true))
        .catch(() => {
          return;
        });
      // add a resolving promise
      await sequence.addAndPromise(
        getPromiseWorker('resolving', false, 10, stub)
      );
      expect(stub.called).to.be.true;
    });
    ...
    it('should process all workers in enqueue order.', async function() {
      ...
      function createStub(resolve?: any, reject?: any, resultWait?: number) {
        ...
        if (resolve) {
          ...
          stub.callsFake(() =>
            wait(resultWait || 0).then(() => Promise.reject(reject))
          );
        }
        stubs.push(stub);
        return stub;
      ...
      for (let i = 0; i < 20; i++) {
        ops.push(() =>
          sequence.addAndPromise(createStub(1, null, Math.random() * 100))
        );
      }
      for (let i = 0; i < 20; i++) {
        ops.push(() =>
          wait(Math.random() * 1000).then(() =>
            sequence.addAndPromise(createStub(2, null, Math.random() * 100))
          )
        );
      }
      for (let i = 0; i < 100; i++) {
        ops.push(() =>
          sequence.addAndPromise(createStub(null, new Error(`Error: ${i}`)))
        );
      }
      for (let i = 0; i < 100; i++) {
        ops.push(() =>
          wait(Math.random() * 1000).then(() =>
            sequence.addAndPromise(createStub(3))
          )
        );
      }
