...
import { LoggerStub } from '@risevision/core-utils/tests/stubs';
...
describe('helpers/protobuf', () => {
  ...
  describe('validate', () => {
    ...
    it('should set lastError, call logger.debug and return false if message.verify returns an error message', () => {
      ...
      expect(fakeLogger.stubs.debug.calledOnce).to.be.true;
      expect(fakeLogger.stubs.debug.firstCall.args).to.be.deep.equal([
  ...
  describe('encode', () => {
    ...
    describe('input/output tests', () => {
      ...
      //   const out = instance.encode(obj, 'blocks.bytes', 'bytesBlock') as Buffer;
      //   expect(out).to.be.deep.equal(buf);
      // });
      //
  ...
  describe('decode', () => {
    ...
    it('should throw if a ProtocolError is caught', () => {
      ...
      expect(() => {
        instance.decode(buf, 'APISuccess');
      }).to.throw('ProtoBuf Protocol Error protoerr');
    });
