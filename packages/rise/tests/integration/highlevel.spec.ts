...
describe('highlevel checks', function() {
  ...
  describe('txs', () => {
    ...
    describe('delegate', () => {
      ...
      it('should not allow delegate name longer than 20 chars', async () => {
        await createRegDelegateTransaction(
          1,
          senderAccount,
    ...
    describe('multiSignature', () => {
      ...
      beforeEach(async () => {
        postsignRequest = initializer.appManager.container.getNamed(
      ...
      it('should keep tx in multisignature tx pool until all signature arrives, even if min is 2', async () => {
        ...
        const signatures = keys.map((k) =>
          ed.sign(txLogic.getHash(signedTx, false, false), {
    ...
    describe('edge-cases', () => {
      it('should react correctly when tx seems to belong to multisignature account while account is not multisign', async () => {
        const tx = await createSendTransaction(1, funds, senderAccount, '1R', {
          requesterPublicKey: senderAccount.publicKey,
        });
        // Blocks should be forged anyway
        expect(blocksModule.lastBlock.height).is.eq(3);
        // txmodule should not find it in db
  ...
  describe('other tests', () => {
    it('transactions/blocks concurrency test 1', async function() {
      ...
      const txs = await Promise.all(
        new Array(1000).fill(null).map((what, idx) =>
          createSendTransaction(0, fundPerTx, senderAccount, '1R', {
            timestamp: idx,
          })
        )
      );
      ...
      for (let i = 0; i < total; i++) {
        ...
        expect(new BigNumber(balance).toNumber()).to.be.eq(
          new BigNumber(funds)
            .minus(fundPerTx * (i + 1))
        ...
        expect(new BigNumber(u_balance).toNumber()).to.be.eq(
          Math.max(
            0,
            new BigNumber(funds)
