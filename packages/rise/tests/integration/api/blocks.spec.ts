...
describe('api/blocks', () => {
  ...
  //       const txs = [
  //         await createSendTransactionV1(0, 1, data[0].account, '1R'),
  //         await createSecondSignTransactionV1(
  //           0,
  //           data[3].account,
  //           createRandomWallet().publicKey
  //         ),
  //         await createRegDelegateTransactionV1(0, data[2].account, 'meoaaw'),
  ...
  //       expect(body.blocks[0].transactions[1].asset).deep.eq(toTransportable(txs[1].asset));
  ...
  describe('/get', () => {
    ...
    it("should return block's transactions with assets per each tx", async () => {
      ...
      expect(body.block.transactions[1].asset).deep.eq(
        toTransportable(txs[1].asset)
      );
  ...
  describe('/getBroadhash', () => {
    ...
    it('should change broadhash and return new one if based on known blocks', async () => {
      ...
      return supertest(initializer.apiExpress)
        ...
        .expect(200)
        .then((response) => {
          expect(response.body.broadhash).to.be.eq('14149481915235850637');
        });
    });
  });
    ...
    it('should return obj with several elements', async () => {
      return supertest(initializer.apiExpress)
        ...
        .then((response) => {
          ...
          expect(response.body.fee).to.be.a('string');
        });
    });
