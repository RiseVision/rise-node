...
export class MultiSignaturesApi {
  ...
  params: {
    ...
    for (const account of accounts) {
      const addresses = account.multisignatures.map((pk) =>
        this.accounts.generateAddressByPublicKey(Buffer.from(pk, 'hex'))
      );
      ...
      items.push({
        ...
        ...{
          multisigaccounts: multisigaccounts.map((m) =>
            filterObject(m.toPOJO(), ['address', 'publicKey', 'balance'])
          ),
        },
      });
    }
      ...
      .map((tx) => {
        ...
        if (senderMultiSig || tx.type === 4) {
          const matchingPubKey = [
            sender.publicKey,
            ...(senderMultiSig
              ? sender.multisignatures.map((pk) => Buffer.from(pk, 'hex'))
              : []),
            ...(tx.type === 4
              ? tx.asset.multisignature.keysgroup.map((pk) =>
                  Buffer.from(pk.substr(1), 'hex')
                )
              : []),
        ...
        const signed =
          tx.senderPublicKey.equals(bufPubKey) ||
          this.multiUtils.isTxSignedByPubKey(tx, bufPubKey);
