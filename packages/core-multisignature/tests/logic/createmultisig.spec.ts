...
import { Accounts2MultisignaturesModel, Accounts2U_MultisignaturesModel, MultiSignaturesModel } from '../../src/models';
import {
  IAccountLogic, IAccountsModel, IAccountsModule,
  ISystemModule,
  ITransactionLogic,
  ITransactionsModel, Symbols
} from '@risevision/core-interfaces/dist/';
import { createContainer } from '@risevision/core-launchpad/tests/utils/createContainer';
import { DBCreateOp, DBRemoveOp, DBUpdateOp, DBUpsertOp, TransactionType } from '@risevision/core-types';
import { MultisigSymbols } from '../../src/helpers';
import { ModelSymbols } from '@risevision/core-models';
import { TXSymbols } from '@risevision/core-transactions';
import { LiskWallet } from 'dpos-offline';
import { toBufferedTransaction } from '@risevision/core-transactions/tests/utils/txCrafter';
...
describe('logic/transactions/createmultisig', () => {
  ...
  beforeEach(async () => {
    ...
    multisigModel           = container.getNamed(ModelSymbols.model, MultisigSymbols.models.model);
  ...
  describe('apply', () => {
    ...
    it('should return correct ops', async () => {
      ...
      for (let i = 0; i < tx.asset.multisignature.keysgroup.length; i++) {
        ...
        expect((first as any).values).deep.eq({ address: w[i].address, publicKey: Buffer.from(w[i].publicKey, 'hex') });
  ...
  describe('undo', () => {
    ...
    describe('returned ops', () => {
      ...
      it('should return proper ops when account was previously multisig (prevtx)', async () => {
        ...
        txFindOne.resolves({ asset: { multisignature: { min: 10, lifetime: 20,
        ...
        for (let i = 0; i < 2; i++) {
          ...
          expect(op1.values).deep.eq({ address: add, publicKey: Buffer.from(pk, 'hex') });
  ...
  describe('afterSave', () => {
    it('should emit on the socket and resolve', async () => {
      ...
      expect(socketIO.sockets.emit.firstCall.args[0]).to.be.equal('multisignatures/change');
