...
export class CoreModule extends BaseCoreModule<AppConfig> {
  ...
  public addElementsToContainer() {
    ...
    this.container.bind(APISymbols.middleware)
  ...
  public async initAppElements() {
    ...
    */
    ...
    controllers.concat(middlewares).forEach((el) => {
      this.container.bind(APISymbols.class).to(el).inSingletonScope().whenTargetNamed( Reflect.getMetadata(Symbols.class, el));
    });
  ...
  public async boot() {
    ...
    await cbToPromise((cb) => this.srv.listen(appConfig.api.port, appConfig.api.address || appConfig.address, cb));
  }
