  ...
  describe('in/out', () => {
    ...
    it('should fail if request is invalid', async () => {
      await expect(createRequest({})).rejectedWith(
        'query - Missing required property: lastBlockId'
      );
      await expect(createRequest({ lastBlockId: 'a,b' })).rejectedWith(
        "lastBlockId - Object didn't pass validation"
      );
    ...
    it('should fail if block does not exist', async () => {
      ...
      await expect(createRequest({ lastBlockId: '10' })).rejectedWith(
        'Block 10 not found!'
      );
    ...
    it('should encode/decode some blocks', async () => {
      ...
      sandbox
        .stub(blocksUtils, 'loadBlocksData')
